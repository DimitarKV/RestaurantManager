...
public class ItemServiceImpl implements ItemService {
    ...
    private final CategoryRepository itemTypeRepository;

    public ItemServiceImpl(ItemRepository itemRepository, CategoryRepository itemTypeRepository) {
    ...
    public void init() {
        if (this.itemRepository.count() == 0) {
            this.itemRepository.saveAllAndFlush(List.of(
                    new Item().setName("ШЕФС БУРГЕР").setPrice(12.9).setImageUrl("https://res.cloudinary.com/dwqf9zolg/image/upload/v1637593051/shefs_burger_rospbo.webp").setDescription("Бриош питка, 100% телешки бъргър, разтопен английски чедър, хрупкав бекон, халапеньо, карамелизиран лук, айсберг, Chefs сос.Всички бургери и стекове се приготвят средно изпечени, освен ако не пожелаете друго!"),
                    ...
                    new Item().setName("САЛАТА “КАЛИНА“  320гр.").setPrice(10.9).setImageUrl("https://res.cloudinary.com/dwqf9zolg/image/upload/v1637599053/salad_domat_patladjan_o9ztgs.jpg").setDescription("Домат, печен патладжан, овче сирене, маслинова паста, пресен спанак и печени орехи"),
//                  new Item().setName("").setPrice(0.0).setImageUrl("").setDescription(""),
                    new Item().setName("").setPrice(0.0).setImageUrl("").setDescription("")
            ));
        }
